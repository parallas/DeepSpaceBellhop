...
public class SettingsOptionSlider : SettingsOption<int>
{
    ...
    private Rectangle KnobBounds => new(
        ...
        + Vector2.UnitX * ((float)(_visualValue - Min) / (Max - Min + StepAmount)) * _bounds.Width
    ...
    public override void Update(bool isSelected)
    {
        ...
        {
            SetValue(MathHelper.Clamp(GetValue() + StepAmount * inputDir, Min, Max));
        }
    ...
    {
        bool highlighted = (_usingMouse && (_bounds.Contains(MainGame.Cursor.ViewPosition) || _mouseSelecting)) || (isSelected && !_usingMouse);
        ...
        float knobX = (float)(_visualValue - Min) / (Max - Min + StepAmount);
        ...
        _sliderMiddleSprite.ScaleX = knobX * _bounds.Width;
        ...
        _knobSprite.Draw(spriteBatch, (_bounds.Location + new Point(4 + MathUtil.FloorToInt(knobX * _bounds.Width), 4)).ToVector2());
